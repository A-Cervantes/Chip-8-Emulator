...
int main(int argc, char const *const *const argv) {
    ...
    while (1) {
        ...
        SDL_Event event;
        while (SDL_PollEvent(
            &event)) { // While there are events to process user quits
        ...
        for (uint32_t calcloop = 0; calcloop < INSTRUCT_SPEED / 60;
             calcloop++) {
            deopcode(&chip);
        }
        ...
        SDL_Rect rectangle = {
            .x = 0, .y = 0, .w = SCALE_FACTOR, .h = SCALE_FACTOR};

        // loop through whole chip display, if it's set to 1 draw white
        // rectangle, otherwise draw black rectangle
